...
################################
def view_tenant(get_tenant_file):
    ...
    try:
        ...
        file= open(get_tenant_file, 'w')
        file.write(r.text)
        file.close()

        tree = ET.parse('C:\Python\ACI\Get_Tenant_ACI.xml')
        root = tree.getroot()
...
def tenant_vrf(tenant_input):
    ...
    try:
        ...
        tree = ET.parse('C:\Python\ACI\Get_Tenant_VRF_ACI.xml')
        root = tree.getroot()
...
def view_bd(tenant_input):
    ...
    try:
        ...
        file = open(get_bd_file, 'w')
        file.write(r.text)
        file.close()

        tree = ET.parse('C:\Python\ACI\Get_BD_ACI.xml')
        root = tree.getroot()
...
def view_app_profiles(tenant_input):
    ...
    try:
        ...
        file = open(get_app_file, 'w')
        file.write(r.text)
        file.close()

        tree = ET.parse('C:\Python\ACI\Get_App_ACI.xml')
        root = tree.getroot()
...
def view_epgs(tenant_input, app_input, get_epg_file):
    ...
    try:
        ...
        file = open(get_epg_file, 'w')
        file.write(r.text)
        file.close()

        tree = ET.parse('C:\Python\ACI\Get_EPG_ACI.xml')
        root = tree.getroot()
...
############################################## Displays current EPGs. Reads APIC via URI, saves ouput to file and then iterates file to find EPGss
    ...
    view_epgs(tenant_input, app_input, get_epg_file)
...
if __name__ == '__main__':
    ...
    get_tenant_file = "C:\Python\ACI\Get_Tenant_ACI.xml"

    contract_file = "C:\Python\ACI\Create_Contract_ACI.xml"

    get_vrf_file = "C:\Python\ACI\Get_Tenant_VRF_ACI.xml"

    bd_file = "C:\Python\ACI\Create_BD_ACI.xml"
    get_bd_file = "C:\Python\ACI\Get_BD_ACI.xml"

    epg_file = "C:\Python\ACI\Create_EPG_ACI.xml"
    get_epg_file = "C:\Python\ACI\Get_EPG_ACI.xml"

    app_file = "C:\Python\ACI\Create_App_ACI.xml"
    get_app_file = "C:\Python\ACI\Get_App_ACI.xml"

    warnings.filterwarnings("ignore")
    apic = " "
    apic_login()  # Initial APIC login. Menu (5) option to switch APIC
    main()
