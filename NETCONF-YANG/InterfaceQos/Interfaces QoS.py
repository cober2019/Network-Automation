...
def view_qos_stats(host):
    ...
    try:
        ...
        for row in c.execute('SELECT DISTINCT Name, PolDirect, Speed FROM IntStatsQoS WHERE Device=?', (host,)):
            print("Interface: {:>0} | Direction:{:<4} | Speed: {:>0} | Last Poll: {:<4}".format(row[0], row[1], row[2], pre_poll_date[0][0]))
            int_headers()
            print("\n")
                ...
                try:
                    ...
                    for row in h.execute('SELECT ClassPkts, ClassBytes, ClassRate, DropPkts, DropBytes, DateTime FROM IntStatsQoSCopy WHERE Device=? AND ClassName=?',(host, class_name)):
                        try:
                            ...
                            time_date_diff = time_diff(time_diff_1)

                            try:
                                class_hist_pack = round((class_pkts - int(row[0])) / time_date_diff, 2)
                                class_hist_bytes = round((class_bytes - int(row[1])) / time_date_diff, 2)
                                class_hist_rate = round((class_rate - int(row[2])) / time_date_diff, 1)
                                ...
                                drop_hist_pkts = round((drop_pkts - int(row[3])) / time_date_diff, 2)
                                drop_hist_bytes = round((drop_bytes - int(row[4])) / time_date_diff, 2)
...
def main():
    ...
    while True:
        ...
        if selection == "1":
            ...
            del_entry(host)
            del_entry_diff(host)
            netconf_session = device_connect(host)
            interfaces(netconf_session, host)
