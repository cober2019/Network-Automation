...
def prefix_parse(v: Dict[str,str] = None, action: Dict[str,str] = None) -> dict:
...
class IpOpsAsr():
    ...
    xml_filter = """<filter xmlns:xc="urn:ietf:params:xml:ns:netconf:base:1.0" 
    ...
    def ip_prefix_list(self) -> None:
        ...
        intf_info = self.session.get(IpOpsAsr.xml_filter)
        ...
        for k, v in init_dict_breakdown.items():
            if not isinstance(v, list):
                if v["action"] == "deny":
                    entry_details = prefix_parse(v=v, action="deny")
                elif v["action"] == "permit":
                    entry_details = prefix_parse(v=v, action="permit")

                self._prefix_list[k].append(entry_details)

            else:
                for statement in v:
                    if statement["action"] == "deny":
                        entry_details = prefix_parse(v=statement, action="deny") # isr vs asr
                    elif statement["action"] == "permit":
                        entry_details = prefix_parse(v=statement, action="permit")
    ...
    def find_overlapping_prefixes(self) -> Any:
    ...
    def send_prefix_list(self, **kwargs: str) -> Any:
        ...
        routing_table = IOSXE.RoutingIos(host=self.host, username=self.username, password=self.password)
        self.routing_table = routing_table.routing
    ...
    def find_dups_internal(self, **kwargs):
